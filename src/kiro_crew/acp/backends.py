...
class Routing(str, Enum):
    ...
    EXTERNAL_POLICY: the decision lives in the adapter's own configuration,
    which Kiro Crew does not own and must therefore probe.
    CLIENT_DELEGATED: the adapter performs no privileged operation itself — it
...
class BackendDescriptor:
    ...
    directory also holds non-secret configuration. ``process_markers`` feed the
    PID-recycle guard, so a backend absent from them is never recognised as ours
    and its tracked entries are pruned instead of reaped.
    """
...
def descriptor_for_registry_adapter(
    ...
    return BackendDescriptor(
        ...
        credential_leaves=(),
        # Used for process-tree matching; an unknown adapter's process name is not
        # known, and a wrong marker matches the wrong process.
        process_markers=(),
        permission_config_id="",
        permission_config_value="",
...
def descriptor_for(backend: str) -> BackendDescriptor:
    ...
    adapter = registry.cached().get(resolved)
...
def selectable_ids(*, refresh_registry: bool = False) -> frozenset[str]:
    """Static backends plus launchable adapters from the cached ACP registry.

    ``refresh_registry`` is used only by the explicit backend-discovery API.
    Config/schema hot paths remain cache-only and therefore never introduce a
    network call for operators who stay on the default Kiro backend.
    """
    from kiro_crew.acp import registry
