"""Codex-specific paths, resolution, and leftover config.toml helpers.
...
``$CODEX_HOME/config.toml`` is not consulted for that decision. The
``approval_policy`` helpers below read the file for diagnosis; they do not
gate session start.
...
# Leftover file-probe classification of ``approval_policy`` values. The live
# Codex gate is session-config ``mode=read-only``, not these file keys.
PERMISSION_BYPASS_POLICIES = frozenset({"never", "on-failure"})
PERMISSION_ROUTED_POLICIES = frozenset({"untrusted", "on-request"})
...
def config_toml_path() -> Path:
    """The adapter's own configuration, including ``approval_policy``."""
    return codex_home() / "config.toml"


def _load_toml(path: Path) -> dict | None:
    """Parse ``path`` as TOML. ``None`` when it cannot be read or parsed.

    ``tomllib`` is stdlib from 3.11; ``tomli`` is declared as a dependency below
    that. A missing parser returns None, which the caller turns into
    INDETERMINATE rather than a permissive default.
    """
    try:
        ...
        try:
            ...
            logger.warning(
                "No TOML parser available, so the leftover Codex approval "
                "policy probe cannot read %s.",
                path,
            )
            return None
    try:
...
def effective_approval_policy(path: Path | None = None) -> str:
    """Return the leftover file-probe's effective ``approval_policy``, or ``""``.

    Diagnosis only: Codex session start does not consult this value. The live
    gate is session-config ``mode=read-only`` after session/new|load. Resolution
    order still mirrors the adapter's own file (a top-level ``profile`` key names
    a ``[profiles.<name>]`` table whose ``approval_policy`` overrides the
    top-level one) because that is what the old probe read, and the history of
    why that reading was wrong depends on it.

    ``""`` means "not determinable", which covers a missing file, an unparseable
    file, a named profile that does not exist, and a policy of a non-string type.
    """
    toml_path = path if path is not None else config_toml_path()
    ...
    if isinstance(selected, str) and selected:
        ...
        if not isinstance(profiles, dict) or selected not in profiles:
            logger.warning(
                "Codex config selects profile %r which is not defined in %s; "
                "the effective approval policy cannot be determined.",
                selected,
                toml_path,
            )
            return ""
        table = profiles.get(selected)
        if not isinstance(table, dict):
            return ""
        policy = table.get("approval_policy")
        # A profile that does not set the key inherits the top-level value.
        if policy is None:
...
def routing_verdict(path: Path | None = None) -> tuple[Verdict, str]:
    """Diagnose the leftover ``approval_policy`` file probe.

    Unused by ``Routing.SESSION_CONFIG`` start enforcement: the live gate
    applies ``mode=read-only`` after session/new|load and never calls this.
    Kept so a host-side read of ``config.toml`` can still name the effective
    policy when diagnosing why an older probe would have answered ROUTED or
    BYPASSED. Not a start requirement.
    """
    toml_path = path if path is not None else config_toml_path()
    policy = effective_approval_policy(toml_path)

    if not policy:
        return (
            Verdict.INDETERMINATE,
            f"no readable approval_policy in {toml_path}",
        )
    if policy in PERMISSION_BYPASS_POLICIES:
        return (
            Verdict.BYPASSED,
            f"approval_policy={policy} auto-approves sandboxed commands",
        )
    if policy in PERMISSION_ROUTED_POLICIES:
        return (Verdict.ROUTED, f"approval_policy={policy}")
    # An unrecognized value is not assumed safe: the adapter may add a policy
    # that bypasses, and defaulting to ROUTED would silently adopt it.
    return (
        Verdict.INDETERMINATE,
        f"unrecognized approval_policy={policy} in {toml_path}",
    )


def remediation_hint(path: Path | None = None) -> str:
    """ACP session config is the gate; ``config.toml`` is not consulted."""
    toml_path = path if path is not None else config_toml_path()
    return (
        "Kiro Crew applies ACP session config mode=read-only after session/new "
        "and session/load, and refuses if the adapter does not advertise that "
        f"value or the write fails. {toml_path} is not consulted for this "
        "decision."
    )
...
__all__ = [
    "CODEX_ACP_BIN",
    "PERMISSION_BYPASS_POLICIES",
    "PERMISSION_ROUTED_POLICIES",
