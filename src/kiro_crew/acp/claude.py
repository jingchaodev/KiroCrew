...
def ensure_routed_settings(work_dir: Path | str) -> bool:
    ...
    """
    path = local_settings_path(work_dir)
    existing_mode = configured_permission_mode(work_dir)
    if existing_mode:
        logger.debug(
    ...
    if raw:
        try:
            ...
            # Malformed file: start from an empty document rather than refuse to
            # seed. The readback probe still decides whether the result routes.
