...
def routing_verdict(backend: str, work_dir: Path | str) -> tuple[Verdict, str]:
    ...
    if descriptor.routing is acp_backends.Routing.EXTERNAL_POLICY:
        from kiro_crew.acp import codex as codex_backend

        return codex_backend.routing_verdict()

    if descriptor.routing is acp_backends.Routing.CLIENT_DELEGATED:
        # The adapter performs no privileged operation itself: filesystem and
        # terminal work come back over ``fs/*`` and ``terminal/*``, and tool calls
        # arrive as permission requests. There is nothing to seed and nothing to
        # read back, because the guarantee is structural rather than configured —
...
def remediation_for(backend: str, work_dir: Path | str) -> str:
    ...
    if descriptor.routing is acp_backends.Routing.EXTERNAL_POLICY:
        ...
        return codex_backend.remediation_hint()
    if descriptor.routing is acp_backends.Routing.UNVERIFIED:
