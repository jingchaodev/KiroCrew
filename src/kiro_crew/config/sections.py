...
class SessionConfig:
    ...
    compaction_method: str = field(
        ...
        metadata=_meta(
            "Compaction Method",
            "The strongest compaction to use when the Auto-Compact Threshold "
            "fires; weaker ones follow automatically and 'native' always runs "
            "last, so a session at the threshold is always compacted. 'native' "
            "has the agent runtime summarize the whole context in place (one "
            "model call, minutes on a large window). 'soft' starts a fresh "
            "session seeded with the recent conversation tail the session replay "
            "carries for the model's window, no model call. 'shake' also carries "
            "an elided digest of what was dropped, on surfaces that can write one "
            "(the dashboard); where no digest can be written (channels) it goes "
            "straight to 'native' rather than drop that history undigested. An "
            "unknown name means 'native'.",
