...
class TestResolutionLadder:
    ...
    def test_adapter_on_path_resolves(
        ...
        argv = codex.resolve_argv()
        assert argv is not None
        assert argv[0].endswith(codex.CODEX_ACP_BIN)
...
class TestAuth:
    ...
    def test_auth_paths_follow_codex_home(self, codex_home: Path) -> None:
        assert codex.auth_json_path() == codex_home / "auth.json"
        assert codex.config_toml_path() == codex_home / "config.toml"
