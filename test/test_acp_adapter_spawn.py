...
class TestGooseResolver:
    def test_argv_is_the_binary_plus_the_acp_subcommand(self, tmp_path: Path) -> None:
        """goose serves ACP from its own binary — there is no adapter package."""
        fake = tmp_path / "goose"
...
class TestSpawnResolvesEachBackendsOwnArgv:
    ...
    def test_kiro_remains_the_final_fallthrough(self) -> None:
        """Harness parity: the added arms must not touch the Kiro path.

        Kiro is spelled as the empty string and is reached by the trailing `else`,
        so it must NOT gain an id comparison of its own here.
        """
        import inspect
        ...
        source = inspect.getsource(AcpClient._spawn)
        assert "ACP_BACKEND_KIRO" not in source
...
class TestSpecAdaptersAreNotToldWeSupportElicitation:
    ...
    def test_initialize_selects_the_set_by_dialect(self) -> None:
        ...
        assert "ACP_CLIENT_CAPABILITIES_SPEC_ADAPTER" in source
        assert "_is_spec_adapter" in source
