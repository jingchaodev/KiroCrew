 ...
 *  hook resolves its target SYNCHRONOUSLY on its first effect, and this card
 *  renders `null` until `GET /api/acp-backends` answers — a registry refresh,
 *  so hundreds of ms at best. It would strip the param and scroll nowhere. The
 *  scroll therefore lives here, where the load is observable. */
export const ACP_BACKEND_ANCHOR = 'acp-adapter'
export const ACP_BACKEND_ROUTE = `/developer?tab=config#${ACP_BACKEND_ANCHOR}`
...
interface Props {
  ...
  onSave: (path: string, value: string) => Promise<void> | void
}

export function AcpBackendCard({ onSave }: Props) {
  const [data, setData] = useState<AcpBackendsPayload | null>(null)
  ...
  const load = useCallback(async () => {
    try {
      ...
      // than an error — the control is not theirs to operate.
      setFailed(true)
    }
  }, [])
  ...
  const choose = (row: AcpBackendRow) => {
    ...
    // Confirmation is required for an experimental backend and only for one:
    // returning to the default is always safe, so a dialog there would be noise
    // that teaches the operator to click through dialogs.
    if (row.experimental) setPending(row)
    else void commit(row)
  }
            ...
            >
              ...
              {row.degraded_count > 0 && (
                <>
                  <button
                    type="button"
                    className="flex items-center gap-1 mt-2 text-[12px] text-muted hover:text-text"
                    aria-expanded={open}
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-2 justify-end">
            <button
              type="button"
              className="px-3 py-1.5 rounded text-sm border border-border-strong"
            ...
            <button
              type="button"
              className="px-3 py-1.5 rounded text-sm font-medium bg-accent text-accent-fg disabled:opacity-40"
              disabled={saving}
