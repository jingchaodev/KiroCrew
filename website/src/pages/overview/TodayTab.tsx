...
function SessionRow({ session, onOpen }: { session: TodaySession; onOpen: (s: TodaySession) => void }) {
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full shrink-0 ${session.running ? 'bg-ok' : 'bg-border-strong'}`}
    ...
    <div className="grid gap-3.5 grid-cols-2 max-[760px]:grid-cols-1" data-testid="today-tab">
      <Card>
        ...
        {today.sessionsError ? (
          ...
          <ErrorNotice message={today.sessionsError.message} askAgent testId="today-sessions-error" />
        ) : today.loading ? (
      ...
      <Card>
        ...
        {today.historyError ? (
          <ErrorNotice message={today.historyError.message} askAgent testId="today-history-error" />
