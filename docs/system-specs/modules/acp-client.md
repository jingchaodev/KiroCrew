...
- **MCP servers**: backend-dependent.
  ...
  - **Spec adapters** (claude / Codex / goose / OpenCode / pi / a ROUTED
    ...
    Delivery requires a `ROUTED` verdict. goose and pi are
    `PERMISSION_REQUEST`; OpenCode is `SEEDED_SETTINGS` (`permission: ask`).
    All three therefore receive Crew servers without the
