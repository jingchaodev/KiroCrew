...
- `"codex"` (`ACP_BACKEND_CODEX`): the public `@agentclientprotocol/codex-acp` adapter. Resolution is `CODEX_ACP_BIN`, mise, then the gateway's augmented PATH. It uses standard ACP protocol v1, receives Kiro Crew's converted MCP server list in `session/new` / `session/load`, switches models through `session/set_config_option`, and steers through `_session/steering`. Install with `npm install -g @agentclientprotocol/codex-acp`; the adapter reuses `~/.codex`, so no Kiro login is required. Kiro Crew omits the unimplemented `elicitation` capability for this backend, making MCP approvals fall back to `session/request_permission`.
...
- `protocolVersion` in the `initialize` request: kiro-cli expects the date string `"2025-08-22"`; standard adapters (Codex and Claude) expect integer `1`.
- Standard adapters skip `session/set_mode` and use `session/set_config_option` (configId `model`) instead of `session/set_model`.
...
| `elicitation` | `{form: {}, url: {}}` for Kiro; omitted for Codex | Kiro keeps the existing forward-compatible declaration. Codex already routes MCP approvals through `elicitation/create` when advertised, so the Codex branch omits it until Kiro Crew has a handler; the adapter then uses `session/request_permission`, which Kiro Crew fully supports. |
