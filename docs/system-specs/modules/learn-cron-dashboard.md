...
- Lock contract: mutators (`add_job`/`update_job`/`remove_job`/`enable_job`) acquire the store lock via a bounded non-blocking spin (`_file_lock`); the loop-resident scheduling paths (timer tick, `run_job`) never take the store lock or `_sync()` on the event loop — they **offload** the locked `_sync()`+drain+snapshot to a worker thread via `asyncio.to_thread` (`_tick_scan_locked` for `_on_timer`, `_synced_snapshot` for `run_job`) and run only the mutation-free due-scan / `_executing` claim on the loop. A manual run is claimed **at call time**: `run_job` is a plain `def` that takes the `_executing` entry and the `(started, "manual")` run meta synchronously while `run_job(job_id)` is evaluated — before the route's `create_task` schedules the wrapper it stores in `_running_tasks`, and before the coroutine's first `await` (the offloaded snapshot, up to a full lock spin) — so `POST /api/crons/{id}/cancel`, whose `cancel()` reads only `_executing`, and the Run guard, which reads `_running_tasks`, agree from the first instant — a claim that follows the snapshot lets Cancel answer 409 "not running" about a run that Run calls "already running" until the snapshot returns. The coroutine releases a claim the store has no job for, and a wrapper cancelled while its snapshot is in flight clears the `_cancelled_jobs` marker `cancel()` left for a `_run_job_isolated` `finally` that never runs. **Every release site is ownership-checked**: that release, `_run_job_isolated`'s `finally` (start stamps, jitter, run meta, `_executing`, tracked task) and the manual wrapper's backstop each pop a job's run state only while the stored meta tuple is still the one their own run installed (identity, not equality), so a newer claim is never released by an older run — `cancel()` and the reaper release the run they found while its `_executing` entry still keeps every other claimant out, and a Run accepted during a released run's still-pending finalizer keeps its claim. **The cancel and reap markers are claim-keyed too** (`_RunMarkers`): `cancel()` and the reaper mark the run whose meta tuple they popped, and a `finally` consumes only the marker set for its own tuple — a marker keyed by job id alone is one marker for two cancellations in flight, so a finalizer stalled in its marker clear consumes the replacement run's marker and that run's `finally` then appends a `failure` row after the `cancelled` row `cancel()` already wrote for it. **The run task is handed its claim**: both dispatchers (`_on_timer`, `_run_claimed_manual`) pass the meta tuple they stored into `_run_job_isolated`, whose first step re-checks that the stored claim is still that tuple and otherwise does nothing but consume its own markers — a `cancel()` landing between `create_task` and that first step pops the claim, marks it, and awaits the process kill before `task.cancel()`, so the task starts inside `cancel()`; a meta read back from the store there is None, the `finally`'s marker lookup misses, and the run is filed as a `failure` beside the `cancelled` row `cancel()` writes, while a task that ran on and stamped its start would leave those stamps behind the release fence. **The on-disk in-flight marker is run-keyed too**: `cron_inflight.write_marker` names the file by the job id AND a per-run token minted in `_run_job_isolated`, and the `finally`'s `clear_marker` removes only that file — a clear by job id alone would take the marker a replacement run wrote while the prior run's cancellation was still unwinding (a whole session teardown for an agent job), and a hard exit during the replacement would then leave the breaker no evidence. **The run's terminal record is taken before the release**: the `finally` computes its history row's status and summary and takes a shallow copy of the job for `_merge_job_result` while the claim is still held, because the release lets a replacement start on the same shared `CronJob` object whose `_execute` resets `last_status` and the result-produced flag — a record read after the release filed a completed run as a failure with no summary and persisted the replacement's blank status as the completed run's. **And the record is merged by run generation**: the store persists `run_generation`, a per-job counter written only by the two terminal merges (`_merge_job_result` stamps it from the record's copy, `_merge_terminal_state_locked` from the number its callers `cancel()` and `_force_reap` pass); a run draws its number at its first step -- or the `cancel()` / reap that takes its claim before that draws one -- from `_next_run_generation`, one above both the persisted value and the highest this process has handed out for the job, so two runs never share a number whatever the clock says (Windows' `time.time()` ticks every ~15.6 ms, so two claims in one tick carry equal timestamps, which no timestamp rule can order), across the `_sync()` reloads that replace the job object before a merge lands, and across restarts. Both merges run in a worker thread behind the release of the run's claim, under a store lock other writers contend for, so a replacement run accepted in that window can complete and merge first; each merge compares its record's generation against the persisted one under the lock and does not apply an older record's fields (debug log) instead of reverting status, error, result and the failure counter to a run that is not the last. Only the FIELDS are fenced: `_merge_job_result` still runs the `delete_after_run` consume for an older record -- the consume is owed by that run's completion, a replacement's cancel writes status fields and retires nothing, and a fired at-job left enabled on disk is due again on every tick -- so control always reaches it and the save; the reaper sweep snapshots the atomically-swapped in-memory job list **cache-only** (no lock, no `_sync`). The hot read paths (`list_jobs`/`get_job`) run on the event loop from many callers (per-connection status push, Slack, apps SDK, MCP) and are **cache-only**: they perform **no filesystem I/O at all** — no lock-file open, no `read_bytes()`, no digest hash, no `_sync`/`_load` — and simply return `list(self._jobs)` (never torn; CPython swaps the list reference atomically). This closes the `no-blocking-call-on-event-loop` hazard: a large `crons.json` can no longer freeze the loop with a synchronous read+hash on every status push **or every timer tick**. Cross-process freshness for these reads is maintained **off-loop**: the in-memory snapshot is `_sync()`-refreshed under the store lock by the timer tick (`_on_timer`, every ≤`_TIMER_POLL_SECS`, in its worker-thread transaction) and by every mutator, so an external write is reflected within one poll interval. A freshly-constructed `CronService` performs one synchronous `_load()` at construction so loop-less callers (MCP/CLI/apps-SDK processes, tests) read on-disk state immediately. That inline load is a whole-file `read_bytes()`+blake2b hash, so an **event-loop** context must NOT use the plain constructor: the gateway builds its service *inside* its running async startup coroutine, where a synchronous construction-time load would block the sole loop (chat/WS/timers/heartbeat). Loop contexts therefore construct via the async factory **`await CronService.create(...)`**, which passes `_defer_initial_load=True` (the constructor does no store I/O) and runs the initial `_load()` in a worker thread via `asyncio.to_thread`; `start()` likewise offloads its `_load()` to a worker thread. `_running` is `False` during both, so neither arms a timer off-loop. Callers that need a **guaranteed cross-process-fresh** read on the loop use the async variants `list_jobs_async()`/`get_job_async()`, which offload the locked `_sync()`+read/hash/parse+snapshot to a worker thread via `asyncio.to_thread` (degrading to the in-memory snapshot under lock contention rather than raising, and draining any deferred timer arm on the loop after the offload). The user-facing `GET /api/crons` handler uses `list_jobs_async()`. **Exhaustive on-loop-`_sync` audit** (enumerated in the `_sync` docstring and enforced by `TestReadPathsLocked` + `TestConstructionLoadOffLoop`): every `_sync()` caller, raw `read_bytes()` site, and initialization/`start()` load is either in a worker thread (`asyncio.to_thread`) or a loop-less CLI/MCP/apps-SDK process — none runs on the gateway event loop. **Write-path (`_save`) invariant**: every `_save()` call site and every structural `self._jobs` mutation holds `_file_lock` and is reached from the loop ONLY via `asyncio.to_thread` — no bare on-loop `_save()` remains. This includes the reaper timeout (`_force_reap`) and user-cancel (`cancel`) terminal paths, which previously mutated the in-memory job and called an unlocked `_save()` directly on the loop: a lost-update race that re-serialized a stale job list and could silently drop a job a concurrent `add_job_async`/`update_job_async` worker had just persisted. Both now offload the locked helper `_merge_terminal_state_locked` (mirroring `_merge_job_result`), which `_sync()`s first so the concurrent write is reloaded and the terminal fields are applied to the disk copy inside one lock transaction. The full writer→lock→loop-entry table lives in the `_save` docstring and is enforced by `TestTerminalStateMergeLocked` (lost-update + off-loop-ticking regressions).
...
On-demand job execution via CLI (`kirocrew cron trigger <job_id>`) and MCP tool (`cron_trigger`). Delegates to `POST /api/crons/{id}/run`. The endpoint returns job name and uses `create_task` for non-blocking execution. If a run for the job is already in flight (tracked in `_running_tasks` or `is_running(job_id)`), the endpoint returns **409** `{"error": "job is already running"}` instead of starting a second overlapping run — the guard is an atomic check-and-set with no `await` between the check and the `_running_tasks` assignment, so overwriting (and thereby orphaning) the in-flight task handle is impossible. A tracked task that has already finished does **not** count as in flight: before the check the handler calls `CronService.discard_finished_run(job_id)`, which drops the run's leftover markers (`_running_tasks`, `_executing`, the start stamps, jitter and run meta) when — and only when — the stored task is `done()`, so a run whose task ends without reaching `_run_job_isolated`'s `finally` cannot make the endpoint refuse the job; a task still running keeps the 409. The reaper sweep (`_reaper_loop`, every `_REAPER_INTERVAL`) applies the same release to every finished task it meets whose start stamp is still present — ahead of its deadline math, not once the run's deadline passes — so the scheduled due-scan, `_next_wake_secs` and `run_job`, which all skip a job in `_executing`, see the job idle again within one sweep instead of passing over every scheduled fire until a manual run or a gateway restart. `CronService.cancel()` is the third site: it calls the same release before its `_executing` guard, so `POST /api/crons/{id}/cancel` against a finished task answers **409** `{"error": "job is not running"}` honestly with the leftovers released, instead of killing nothing, recording a `cancelled` history row for a run that ended long ago, and leaving `_cancelled_jobs` set for a `finally` that never runs — which made the job's next real run skip its merge and history as if cancelled. Unknown job → 404. This 409 propagates to the dashboard "Run now" button, the CLI `trigger`, and the `cron_trigger` MCP tool, which surface it as an "already running" outcome rather than a duplicate launch. Both CLI and MCP paths include SEL audit logging.
...
- **Source-provider ownership**: before an owner is configured, a signed machine-local bootstrap subject (`local-app` / `local-startup`) IS the owner for source reads and mutations alike — the same rule `is_owner_dashboard_request` applies to every other owner-gated dashboard surface, the secrets vault included. `owner_id` is only ever populated from `KIROCREW_OWNER_ID`, so scoping the allowance to reads left an install that never configures Slack able to read a pull request but not act on it, with live buttons that refused and pointed at an unrelated Slack setting. Once an owner is configured every operation requires an exact match. App tokens, unauthenticated requests, and non-local subjects always receive the generic forbidden body, so the response never discloses the install's owner state.
...
- **Overview** (`/overview`) — `StatCard` components (with skeleton loading) + tabbed management console:
  - **Today** (summary card in the 2×2 grid beside Usage / WakaTime / Memory; `?view=today` drill-in, `pages/overview/TodayTab.tsx`): what was worked on today, with no model call. The card counts sessions ACTIVE in the browser's local day — live slots from redux plus the archived rows `GET /api/sessions?exclude_open=1` returns, a key present in both counted once with the live slot winning; incognito and temporary sessions never listed — and states the running count, the last activity time, one chip per folder and the number of entries already in today's history file. This is a different set from the Usage card's sessions STARTED today (`usage.py`). The drill-in lists the sessions grouped by folder (`GET /api/chat/folders`, folders ordered by their newest session, unfiled last, a `folder_id` naming no folder treated as unfiled; a click opens the session through `switchSlot` / `resumeFromHistory` like the command palette) beside the day's memory entries from `GET /api/memory/history?date=<browser-local YYYY-MM-DD>`, split on the `#### HH:MM TZ` blocks `append_history` writes. The memory half lags the sessions half by design: its empty state names the consolidation window read from `GET /api/memory/settings` (`history_idle_hours`) and its "Open Memory" action switches to `?view=memory`, where "Summarize now" already lives — no second summarize control. Renders in both memory modes; the vector-active gate that hides the editable Daily history card on the Memory tab is untouched. Exact message counts only for live slots (an archived row's count is a size estimate, so it shows last-active only).
