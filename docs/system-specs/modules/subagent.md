...
4. `_run_inner()` resolves `parent_policy` (parent session → YOLO fallback → config fallback), creates session `subagent:{id}` via `SessionManager.get_or_create(approval_policy=parent_policy)` — policy is persisted on the new session. Dedicated (non-shared) children pin `acp_backend` to the live parent harness (`SessionManager.live_harness`) so a Settings switch cannot redirect `spawn_run` onto a different adapter. The child's model is `resolve_usable_model` against that parent's advertised set: `"auto"` and ids the parent did not advertise are omitted so the child inherits the session default. Parentless / cron spawns keep the factory snapshot. Shared-runtime multiplexing stays kiro-only.
